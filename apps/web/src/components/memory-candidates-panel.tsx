...
export function MemoryCandidatesPanel({ visivel = true }: { visivel?: boolean }) {
  ...
  return (
    <Card>
      <button onClick={() => setAberto(!aberto)} className="flex w-full items-center">
        <PanelTitle>Memórias a confirmar</PanelTitle>
        <span className="ml-auto font-mono text-[10px]" style={{ color: "var(--color-gold)" }}>
          {candidatos.length} {aberto ? "▾" : "▸"}
        </span>
      </button>
          ...
          {candidatos.map((c) => (
            <div key={c.id} className="rounded border p-2" style={{ borderColor: "var(--color-line)" }}>
              {editando?.id === c.id ? (
                ...
                <div className="text-[12px]">{c.fato}</div>
              )}

              <div className="mt-1 flex flex-wrap gap-2 text-[10px]" style={{ color: "var(--color-ink-dim)" }}>
              ...
              {c.evidencia && (
                <div className="mt-1 text-[10px] italic" style={{ color: "var(--color-ink-dim)" }}>
                  “{c.evidencia}”
                </div>
              )}

              {c.parecidaCom && (
                <div className="mt-1 text-[10px]" style={{ color: "var(--color-ink-dim)" }}>
