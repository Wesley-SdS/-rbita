// Migrada do Next em paridade (apps/web/src/app/api/knowledge/graph/route.ts).
...
export async function GET(_req: Request, ctx: RouteCtx) {
  ...
  const nodesRaw = await db.execute(sql`
    ...
    ORDER BY created_at DESC LIMIT ${cfg["graph.nodeLimit"]}
  `);
  ...
  if (nodes.length > 1) {
    // arestas SÓ entre os mesmos nós exibidos (CTE), evita o cross-join
    // quadrático sobre todas as memórias do usuário.
    const raw = await db.execute(sql`
      WITH n AS (
        SELECT id, embedding FROM memory WHERE user_id = ${uid} ORDER BY created_at DESC LIMIT ${cfg["graph.nodeLimit"]}
      )
      ...
      ORDER BY sim DESC LIMIT ${cfg["graph.edgeLimit"]}
    `);
