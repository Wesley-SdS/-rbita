...
**Voz**
- [x] **"Ei Órbita às vezes funciona às vezes não" → wake word NO APARELHO (2026-07-21)** — causa raiz: o wake dependia do Render (streaming contínuo do mic p/ o Vosk remoto), e o free tier **hiberna** (~40s de cold-start) → intermitente. **Fix** (`lib/voice/speech.ts` + `use-voice.ts`): novo `LocalWake` usa a **Web Speech API no próprio aparelho** — reconhecedor contínuo escuta "Ei Órbita" localmente, sem servidor, sem cold-start; ao detectar, cede o mic e dispara o ditado do comando, religando a escuta no fim. O Render vira **fallback** só p/ navegadores sem a API. Coordenação mic↔wake↔ditado via `pause()`/`resume()`. **Bug pego no teste**: o `\b` do JS não trata "ó" como caractere de palavra → `matchesWake` agora casa no texto **sem acento** (NFD). Teste `speech.test.ts` (gatilho, dentro de frase, e negativos como "orbital"/"suborbita"). ⚠️ Reconhecimento contínuo precisa de mic real (não testável no harness) — confirmar no device; no mobile pode religar entre frases.
