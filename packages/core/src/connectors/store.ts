...
export async function refreshConnectionToken(cid: ConnectorId, userId: string, refreshToken: string): Promise<string> {
  const def = getConnector(cid);
...
export async function getAccessToken(cid: ConnectorId, userId: string): Promise<string | null> {
  ...
  if (expiringSoon && row.refreshTokenEnc) {
    try {
      return await refreshConnectionToken(cid, userId, decryptSecret(row.refreshTokenEnc));
    } catch {
